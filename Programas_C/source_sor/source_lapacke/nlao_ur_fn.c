...
double emethod(double *xe, double *xs, int dim)
{
    ...
    cblas_daxpy(n, MINUS, xe+1, incx, error_vec+1, incy);  // error_vec <-- error_vex - xe
...
{
    printf("Successive Over-Relaxation Method with CBLAS and LAPACKE\n");
    printf("error: %.4e, time: %.2e s, iter-max(s): %d\n", error_method, time, iter_max);
}