...
int main(int argc, char const *argv[])
{
    ...
    q_mat = dmatrix(1, dim, 1, dim+1);
    h_mat = dmatrix(1, dim+1, 1, dim);

    r_vec = dvector(1, dim);
    be1_vec = dvector(1, dim+1);
    vj_vec = dvector(1, dim);
    qj_vec = dvector(1, dim);
    sn_vec = dvector(1, dim);
    cs_vec = dvector(1, dim);
    ...
    free_dmatrix(h_mat, 1, dim, 1, dim);
    free_dmatrix(q_mat, 1, dim, 1, dim);
    ...
    free_dvector(cs_vec, 1, dim);
    free_dvector(sn_vec, 1, dim);
    free_dvector(qj_vec, 1, dim);
    free_dvector(vj_vec, 1, dim);
    free_dvector(be1_vec, 1, dim);
