...
int main(int argc, char const *argv[])
{
    ...
    double **d_mat, **l_mat, **u_mat, **t_mat, **invt_mat;
	double *f_vec, *y_vec, *xs_method;
	...
	{
		/* computes U*x(k) */
		matvecmul(y_vec, 1.0, u_mat, xp_vec, dim, dim); /* y_vec <-- u_mat*xp_vec */

		/* computes b - U*x(k) */
		vecadd(f_vec, b_vec, MINUS, y_vec, dim); /* f_vec <-- b - y_vec */

		/* computes x(k+1) = inv(D+L)*(b - U*x(k)) */
		matvecmul(xs_method, 1.0, invt_mat, f_vec, dim, dim); /* xs_method <-- invt_mat*f_vec */
